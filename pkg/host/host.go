...
func (h *Host) BindDeviceDriver(pciAddress string, config *configapi.VfConfig) (string, error) {
	if config.Driver == "" {
		klog.V(2).Info("BindDeviceDriver(): no driver specified, skipping", "device", pciAddress)
		return "", nil
	}
	...
	if config.Driver == "default" {
		klog.V(2).Info("BindDeviceDriver(): binding device to default driver", "device", pciAddress)
	...
	klog.V(2).Info("BindDeviceDriver(): binding device to driver", "device", pciAddress, "driver", config.Driver)
...
func (h *Host) RestoreDeviceDriver(pciAddress string, originalDriver string) error {
	if originalDriver == "" {
		klog.V(2).Info("RestoreDeviceDriver(): no original driver, binding to default", "device", pciAddress)
		return h.BindDefaultDriver(pciAddress)
	}

	klog.V(2).Info("RestoreDeviceDriver(): restoring device to original driver", "device", pciAddress, "driver", originalDriver)
...
func (h *Host) BindDefaultDriver(pciAddress string) error {
	klog.V(2).Info("BindDefaultDriver(): binding device to default driver", "device", pciAddress)
	...
	if curDriver != "" {
		...
		if !h.IsDpdkDriver(curDriver) {
			klog.V(2).Info("BindDefaultDriver(): device already bound to default driver",
...
func (h *Host) BindDriverByBusAndDevice(device, driver string) error {
	klog.V(2).Info("BindDriverByBusAndDevice(): bind device to driver",
	...
	if curDriver != "" {
		if curDriver == driver {
			klog.V(2).Info("BindDriverByBusAndDevice(): device already bound to driver",
				"device", device, "driver", driver)
			return nil
		}
...
func (h *Host) UnbindDriverByBusAndDevice(device string) error {
	klog.V(2).Info("UnbindDriverByBusAndDevice(): unbind device driver for device", "device", device)
	driver, err := h.GetDriverByBusAndDevice(device)
	if err != nil {
		return err
	}
	if driver == "" {
		klog.V(2).Info("UnbindDriverByBusAndDevice(): device has no driver", "device", device)
		return nil
	}
...
func (h *Host) GetDriverByBusAndDevice(device string) (string, error) {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			klog.V(2).Info("GetDriverByBusAndDevice(): driver path for device not exist", "device", device)
			return "", nil
		}
		klog.Error(err, "GetDriverByBusAndDevice(): error getting driver info for device", "device", device)
		return "", err
	}
	klog.V(2).Info("GetDriverByBusAndDevice(): driver for device", "device", device, "driver", driverInfo)
...
func (h *Host) setDriverOverride(device, override string) error {
	...
	if override != "" {
		klog.V(2).Info("setDriverOverride(): configure driver override for device", "device", device, "driver", override)
		overrideData = []byte(override)
	} else {
		klog.V(2).Info("setDriverOverride(): reset driver override for device", "device", device)
		overrideData = []byte("\x00")
	}
	err := os.WriteFile(driverOverridePath, overrideData, os.ModeAppend)
	if err != nil {
		klog.Error(err, "setDriverOverride(): fail to write driver_override for device",
...
func (h *Host) IsKernelModuleLoaded(moduleName string) bool {
	...
	for _, line := range lines {
		if strings.HasPrefix(line, moduleName+" ") || line == moduleName {
			klog.V(2).Info("IsKernelModuleLoaded(): module is loaded", "module", moduleName)
			return true
		}
	}

	klog.V(2).Info("IsKernelModuleLoaded(): module is not loaded", "module", moduleName)
	return false
}
...
func (h *Host) LoadKernelModule(moduleName string) error {
	klog.V(2).Info("LoadKernelModule(): loading kernel module", "module", moduleName)

	cmd := exec.Command("chroot", "/proc/1/root", "modprobe", moduleName)
	output, err := cmd.CombinedOutput()
	if err != nil {
		klog.Error(err, "LoadKernelModule(): failed to load kernel module",
	...
	klog.V(2).Info("LoadKernelModule(): successfully loaded kernel module", "module", moduleName)
	return nil
}
...
func (h *Host) EnsureDpdkModuleLoaded(driver string) error {
	if !h.IsDpdkDriver(driver) {
		klog.V(2).Info("EnsureDpdkModuleLoaded(): driver is not a DPDK driver, skipping module check", "driver", driver)
		return nil
	}
	...
	for _, moduleName := range modulesNames {
		if h.IsKernelModuleLoaded(moduleName) {
			klog.V(2).Info("EnsureDpdkModuleLoaded(): kernel module already loaded", "driver", driver, "module", moduleName)
		} else {
	...
	if len(modulesToLoad) == 0 {
		klog.V(2).Info("EnsureDpdkModuleLoaded(): all required modules already loaded", "driver", driver)
		return nil
	}
	...
	for _, moduleName := range modulesToLoad {
		klog.Info("EnsureDpdkModuleLoaded(): loading kernel module for DPDK driver", "driver", driver, "module", moduleName)
		if err := h.LoadKernelModule(moduleName); err != nil {
			klog.Error(err, "EnsureDpdkModuleLoaded(): failed to load module", "driver", driver, "module", moduleName)
		...
		if !h.IsKernelModuleLoaded(moduleName) {
			...
			klog.Error(err, "EnsureDpdkModuleLoaded(): module verification failed", "driver", driver, "module", moduleName)
			errors = append(errors, err)
		} else {
			klog.Info("EnsureDpdkModuleLoaded(): successfully loaded kernel module", "driver", driver, "module", moduleName)
		}
	}
...
func (h *Host) EnsureVhostModulesLoaded() error {
	...
	for _, moduleName := range modulesNames {
		if h.IsKernelModuleLoaded(moduleName) {
			klog.V(2).Info("EnsureVhostModulesLoaded(): kernel module already loaded", "module", moduleName)
		} else {
			modulesToLoad = append(modulesToLoad, moduleName)
		}
	}
	...
	// If all modules are already loaded, return early
	if len(modulesToLoad) == 0 {
		klog.V(2).Info("EnsureVhostModulesLoaded(): all required vhost modules already loaded")
		return nil
	}
	...
	var errors []error
	for _, moduleName := range modulesToLoad {
		klog.Info("EnsureVhostModulesLoaded(): loading kernel module for vhost functionality", "module", moduleName)
		if err := h.LoadKernelModule(moduleName); err != nil {
			klog.Error(err, "EnsureVhostModulesLoaded(): failed to load module", "module", moduleName)
			errors = append(errors, fmt.Errorf("failed to load module %s: %w", moduleName, err))
			continue
		}
		...
		if !h.IsKernelModuleLoaded(moduleName) {
			err := fmt.Errorf("module %s was not loaded after LoadKernelModule call", moduleName)
			klog.Error(err, "EnsureVhostModulesLoaded(): module verification failed", "module", moduleName)
			errors = append(errors, err)
		} else {
			klog.Info("EnsureVhostModulesLoaded(): successfully loaded kernel module", "module", moduleName)
		}
	}
