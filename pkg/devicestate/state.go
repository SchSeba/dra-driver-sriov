...
func (s *DeviceStateManager) PrepareDevicesForClaim(ctx context.Context, claim *resourceapi.ResourceClaim) (drasriovtypes.PreparedDevices, error) {
	...
	preparedDevices, err := s.prepareDevices(ctx, claim, resultsConfig)
	if err != nil {
...
func (s *DeviceStateManager) prepareDevices(ctx context.Context,
	...
	for _, result := range claim.Status.Allocation.Devices.Results {
		...
		preparedDevice, err := s.applyConfigOnDevice(ctx, claim, config, &result)
		if err != nil {
...
func (s *DeviceStateManager) applyConfigOnDevice(ctx context.Context, claim *resourceapi.ResourceClaim, config *configapi.VfConfig, result *resourceapi.DeviceRequestAllocationResult) (*drasriovtypes.PreparedDevice, error) {
	...
	if config.NetAttachDefName != "" {
		netAttachDefNamespace = config.NetAttachDefName
	}
	...
	// TODO: implement driver change

	// create environment variables
	envs := []string{
		fmt.Sprintf("VF_DEVICE_%s=%s", strings.ReplaceAll(result.Device, "-", "_"), *deviceInfo.Attributes[consts.AttributePciAddress].StringValue),
		fmt.Sprintf("NET_ATTACH_DEF_NAME=%s", config.NetAttachDefName),
	}
	edits := &cdispec.ContainerEdits{
		Env: envs,
	}

	// TODO: implement adding volume mounts to the container edits
...
// TODO: Implement this
// In here we just need to revert the driver configuration moving back the network interface to the original state
// is done in the NRI hook for PodSandboxRemove
